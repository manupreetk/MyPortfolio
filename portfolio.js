...
formcontrol.addEventListener('submit', function (event) {
    ...
    const dummydatabase= JSON.parse(localStorage.getItem('dummydatabase')) || [];  // || is OR, && is AND // YE EK HI LATEST DATA DIKHA RHA H PREVIOUS NHI
    ...
    alert("Thank you for your message, I'll get in touch with you.");
    this.reset();
...
//TOGGLE BUTTON
const toggletheme= document.getElementById("toggle-butn");
...
//USER MESSAGES
//when you write the correct admin username and pw the msgs should show
function displayStoredMsgs(){
    //get all responses from dummydatabas to show here

    const responsecontainer=document.getElementById("user-msgs");
    const dummydatabase= JSON.parse(localStorage.getItem('dummydatabase')) || [];

    //to get the msgs from DB we willarray have to loop through the list and put in the div
    dummydatabase.forEach(response=>{
        const responseElement = document.createElement('div'); //it will create a div tag or is div me niche wale tags dalne h
